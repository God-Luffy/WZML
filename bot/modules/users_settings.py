...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            'lprefix': ['Set Filename Prefix is the Front Part attacted with the Filename of the Upload Files.', 'Send Set Filename Prefix. \n<b>Timeout:</b> 60 sec'],
            'lsuffix': ['Set Filename Suffix is the End Part attached with the Filename of the Upload Files', 'Send Set Filename Suffix. \n<b>Timeout:</b> 60 sec'],
            'lremname': ['Set Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Upload Files', 'Send Set Filename Remname. \n<b>Timeout:</b> 60 sec'],
            'lcaption': ['Set Caption is the Custom Caption on the Files Uploaded by the bot', 'Send Set Caption. You can add HTML tags. \n<b>Timeout:</b> 60 sec'],
            'ldump': ['Custom Files User Dump for Personal Use as a Storage.', 'Send Custom Dump Channel ID\n➲ <b>Format:</b> \ntitle chat_id/@username\ntitle2 chat_id2/@username2. \n\n<b>NOTE:</b>Make Bot Admin in the Channel else it will not accept\n<b>Timeout:</b> 60 sec'],
            ...
            'split_size': ['Upload Splits Size is the size to split the Uploaded File before uploading', f'Send Upload split size in any comfortable size, like 2Gb, 500MB or 1.46gB. \n<b>PREMIUM ACTIVE:</b> {IS_PREMIUM_USER}. \n<b>Timeout:</b> 60 sec'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton("Upload Settings", f"userset {user_id} leech")
        ...
        buttons.ibutton(f"{'✅️' if user_dict.get('split_size') else ''} Set Splits", f"userset {user_id} split_size")

        lcaption = 'Not Exists' if (val:=user_dict.get('lcaption', config_dict.get('LEECH_FILENAME_CAPTION', ''))) == '' else val
        buttons.ibutton(f"{'✅️' if lcaption != 'Not Exists' else ''} Set Caption", f"userset {user_id} lcaption")

        lprefix = 'Not Exists' if (val:=user_dict.get('lprefix', config_dict.get('LEECH_FILENAME_PREFIX', ''))) == '' else val
        buttons.ibutton(f"{'✅️' if lprefix != 'Not Exists' else ''} Set Prefix", f"userset {user_id} lprefix")

        lsuffix = 'Not Exists' if (val:=user_dict.get('lsuffix', config_dict.get('LEECH_FILENAME_SUFFIX', ''))) == '' else val
        buttons.ibutton(f"{'✅️' if lsuffix != 'Not Exists' else ''} Set Suffix", f"userset {user_id} lsuffix")

        lremname = 'Not Exists' if (val:=user_dict.get('lremname', config_dict.get('LEECH_FILENAME_REMNAME', ''))) == '' else val
        buttons.ibutton(f"{'✅️' if lremname != 'Not Exists' else ''} Set Remname", f"userset {user_id} lremname")

        buttons.ibutton("Custom Dump", f"userset {user_id} ldump")
        ...
        if key == 'rcc':
            ...
            text += f"➲ <b>Custom Split Size :</b> <i>{set_exist}</i>\n\n"
